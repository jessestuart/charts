...
Simplify etcd cluster configuration and management.

**DISCLAIMER:** While this chart has been well-tested, the etcd-operator is
still currently in beta. Current project status is available
...
* **Suggested:** PV provisioner support in the underlying infrastructure to
  support backups
...
Before you create an etcd cluster, the TPR must be installed by the operator, so
this option is ignored during helm installs, but can be used in upgrades.
...
`kubectl apply` for TPRs is fixed see
...
The following table lists the configurable parameters of the etcd-operator chart
...
If the output contains "beta" or both "alpha" and "beta" you can may install
rbac by default, if not, you may turn RBAC off as described below.
